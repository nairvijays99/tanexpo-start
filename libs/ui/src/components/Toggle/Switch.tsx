import type { ThemedStyle } from "@libs/theme";
import { $styles, useAppTheme } from "@libs/theme";
...
function SwitchInput(props: SwitchInputProps) {
  ...
  return (
    <View style={[$inputOuter, { backgroundColor: offBackgroundColor }, $outerStyleOverride]}>
      ...
      <SwitchAccessibilityLabel {...props} labelSide="on" />
      <SwitchAccessibilityLabel {...props} labelSide="off" />
 ...
 */
function SwitchAccessibilityLabel(props: SwitchInputProps & { labelSide: "on" | "off" }) {
  const { on, disabled, status, accessibilityMode, labelSide, innerStyle, detailStyle } = props;
  ...
  const shouldLabelBeVisible = (on && labelSide === "on") || (!on && labelSide === "off");
  ...
  return (
    <View style={$switchAccessibilityStyle}>
      {accessibilityMode === "text" && shouldLabelBeVisible && (
        <View
          style={[
            labelSide === "on" && $switchAccessibilityLine,
            labelSide === "on" && { backgroundColor: color },
            labelSide === "off" && $switchAccessibilityCircle,
            labelSide === "off" && { borderColor: color },
          ]}
        />
      )}
      ...
      {accessibilityMode === "icon" && shouldLabelBeVisible && (
        <Image
          ...
          source={labelSide === "off" ? iconRegistry.hidden : iconRegistry.view}
        />
      )}
    </View>
  );
